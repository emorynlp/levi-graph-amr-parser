...
import elit
from elit.common.constant import HANLP_URL, HANLP_VERBOSE
...
from elit.common.io import save_pickle, load_pickle, eprint
...
def hanlp_home():
    """ Home directory for ELIT resources.
...
def get_resource(path: str, save_dir=None, extract=True, prefix=HANLP_URL, append_location=True, verbose=HANLP_VERBOSE):
    """Fetch real path for a resource (model, corpus, whatever) to :meth:`elit.utils.io_util.hanlp_home`.
    ...
    Args:
      ...
      prefix: A prefix when matched with an URL (path), then that URL is considered to official. For official resources,
        they will not go to a folder called ``thirdparty`` under :const:`~elit.common.constants.IDX`.
