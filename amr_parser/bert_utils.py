...
class BertEncoder(BertModel):
    ...
    def forward(self, input_ids, token_subword_index=None):
        ...
        encoded_layers, pooled_output = super(BertEncoder, self).forward(
            input_ids, attention_mask, token_type_ids).to_tuple()
...
class TransformerEncoder(torch.nn.Module):
    ...
    def forward(self, input_ids, token_subword_index=None):
        ...
        encoded_layers, pooled_output = self.encoder.forward(input_ids, attention_mask, token_type_ids)[0], None
        if token_subword_index is None:
            return encoded_layers[:, 1:-1], pooled_output
        else:
            return self.average_pooling(encoded_layers, token_subword_index), pooled_output
...
def load_bert(bert_path):
    if 'electra' in bert_path:
        bert_encoder = AutoModel.from_pretrained(bert_path)
        bert_encoder = TransformerEncoder(bert_encoder)
    else:
        bert_encoder = BertEncoder.from_pretrained(bert_path)
