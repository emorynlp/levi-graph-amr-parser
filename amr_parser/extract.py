...
from elit.datasets.parsing.amr import levi_amr
        ...
        with open(file_path, encoding='utf-8') as f:
            for line in f:
                ...
                if line.startswith('# ::id '):
                    ...
                    amr = AMR.parse_AMR_line(graph_line, do_reverse=do_reverse)
                    myamr = AMRGraph(amr)
...
def read_file(filename, do_reverse=True):
    ...
    for _tok, _lem, _pos, _ner, _myamr in AMRIO.read(filename, do_reverse=do_reverse):
        token.append(_tok)
        lemma.append(_lem)
        pos.append(_pos)
        ner.append(_ner)
...
if __name__ == "__main__":
    ...
    amrs, token, lemma, pos, ner = read_file(args.train_data, do_reverse=do_reverse)
    ...
    for i in range(10):
        ...
        for amr, lem, tok in zip(amrs, lemma, token):
            ...
            if args.levi_graph is True:
                concept, edge = levi_amr([concept], [edge])
